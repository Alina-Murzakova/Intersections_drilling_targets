...
data_file = "База перспективного ПФ (15.02).xlsx"  # Файл для расчета
...
def target(df, data_file, distance, diff_depth, win, pb, label_4):
    ...
    name_columns = pd.MultiIndex.from_tuples(
        [('', '', 'Команда'), ('', '', 'Месторождение'), ('', '', 'КП'), ('', '', 'Объект'), ('', '', 'Назначение'),
         ('', '', 'Скважина - Забой'),
         ('1й ствол', 'T1', 'X'), ('', '', 'Y'), ('', '', 'Z'), ('', 'T3', 'X'), ('', '', 'Y'), ('', '', 'Z'),
         ('', '', 'Комментарий'),
         ('2й ствол', 'T1', 'X'), ('', '', 'Y'), ('', '', 'Z'), ('', 'T3', 'X'), ('', '', 'Y'), ('', '', 'Z'),
         ('', 'Пересечения', 'скв (команда/куст/объект)')])

    if list(df)[-1] == 'Unnamed: 18':  # последний столбец не имел заголовка
        df['Unnamed: 18'] = np.nan
        df = df.rename(
            columns={'X': x1, 'Y': y1, 'Z': z1, 'X.1': x3, 'Y.1': y3, 'Z.1': z3, 'X.2': x11, 'Y.2': y11, 'Z.2': z11,
                     'X.3': x33, 'Y.3': y33, 'Unnamed: 18': z33})  # Переименование столбца
    else:
        df = df.rename(
            columns={'X': x1, 'Y': y1, 'Z': z1, 'X.1': x3, 'Y.1': y3, 'Z.1': z3, 'X.2': x11, 'Y.2': y11, 'Z.2': z11,
                     'X.3': x33, 'Y.3': y33, 'Z.3': z33})  # Переименование столбца
    ...
    for coord in list_coord:
        ...
        df[coord] = np.where((df[coord] < 0), df[coord] * (-1),
                             df[coord])  # замена отрицательных на положительные координаты
    ...
    for field_name in list_fields:
            ...
            for n in start_well_hole_current:
                ...
                for j in range(len(df_field)):
                    ...
                    for k in start_well_hole_round:
                        ...
                        if intersect(x1_current, y1_current, x3_current, y3_current, x1_round, y1_round, x3_round,
                                     y3_round) and i != j:
                            ...
                            list_wells_intersect.append(
                                str(df_field.iloc[j, 5]) + ' (' + str(df_field.iloc[j, 0]) + '/ ' + str(
                                    df_field.iloc[j, 2]) + '/ ' + str(df_field.iloc[j, 3]) + ')')
                        ...
                        elif min_lenght(x1_current, y1_current, x3_current, y3_current, x1_round, y1_round, x3_round,
                                        y3_round) < distance and i != j:
                            num_intersections += 1
                            # print(f'скважина текущая - {df_field.iloc[i, 5]}')
                            # print(f'скважина рядом - {df_field.iloc[j, 5]}')
                            list_wells_near.append(
                                str(df_field.iloc[j, 5]) + ' (' + str(df_field.iloc[j, 0]) + '/ ' + str(
                                    df_field.iloc[j, 2]) + '/ ' + str(df_field.iloc[j, 3]) + ')')
                            list_intersection_teams.add(df_field.iloc[i, 0])
                            list_intersection_teams.add(df_field.iloc[j, 0])
                ...
                plt.plot([df_field.iloc[i, n], df_field.iloc[i, n + 3]],
                         [df_field.iloc[i, n + 1], df_field.iloc[i, n + 4]], c=color)
        ...
        statistics = statistics._append({'Месторождение': field_name, 'Количество пересечений': num_intersections / 2,
                                         'Пересекающиеся команды': ", ".join(map(str, list(list_intersection_teams)))},
                                        ignore_index=True)
        ...
        if len(df_intersection) > 0:
            ...
            sht.pictures.add(fig, name='Plot', update=True, left=sht.range("W1").left,
                             top=sht.range("W1").top)  # добавление графика matplotlib в эксель
    ...
    # сохранение нового эксель в той же директории, что и исходный файл
    new_wb.save(str(os.path.join((dir_result + "/"), os.path.basename(data_file))).replace(".xlsx",
                                                                                           "") + "_out.xlsx")
    # app1.kill()
    app1.quit()
    del app1
